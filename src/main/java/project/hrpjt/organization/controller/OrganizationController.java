...
public class OrganizationController {
    ...
    @PostMapping("/role-adm/organization") //조직 추가
    @ApiOperation(
            value = "조직 추가",
            notes = "새로운 조직을 추가한다.")
    ...
    @GetMapping("/role-emp/organization") // 조직도 조회
    @ApiOperation(
            value = "조직도 조회",
...
//  EMPLOYEE -> 본인 부서만 조회 가능.
    ...
    @GetMapping("/role-emp/organization/employees")
...
//  상위 조직 폐쇄 시 하위 조직까지 모두 종료일을 현재일 -1일로 업데이트함.
    @DeleteMapping("/role-adm/organization")
    @ApiOperation(
            value = "조직 폐쇄",
...
//  시스템 관리자 -> 모든 정보 수정 가능.
    @PutMapping("/role-lead/organization")
    @ApiOperation(
            value = "조직 정보 수정",
